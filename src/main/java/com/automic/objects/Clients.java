...
public class Clients extends ObjectTemplate {
	...
	public void displayClientList(ArrayList<ClientListItem> clients) throws TimeoutException, IOException {
		...
		// LOGGER.info("Client:[Client Name]:[Client Title]:[Client Active]:[# Of Objects In Client]:[# of Users In Client]");
		logWithFormat(LOGGER, "\n%-8s, %-30s, %-9s, %-18s, %-16s, %-14s, %-10s\n", "[Client]", "[Client Title]", "[Active]",
				"[Objects Defined]", "[Users Defined]", "[Activities]", "[Timezone]");
		for (int i = 0; i < clients.size(); i++) {
			...
			// LOGGER.info("Client:"+ClientName+":"+ClientTitle+":"+isClientActive+":"+NumberOfObjects+":"+NumberOfUsers);
			logWithFormat(LOGGER, "%-8s, %-30s, %-9s, %-18s, %-16s, %-14s, %-10s\n", ClientName, ClientTitle, isClientActive,
					NumberOfObjects,
					NumberOfUsers, NumberOfActivities, ClientTZ);
		}
	}
