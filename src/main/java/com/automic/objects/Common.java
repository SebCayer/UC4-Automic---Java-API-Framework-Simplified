...
public class Common extends ObjectTemplate {
	...
	public void restoreObjectVersions(VersionControlListItem VersionControlObject) throws IOException {
		...
		if (req.getMessageBox() != null) {
			LOGGER.info(" \t %% " + req.getMessageBox().getText().toString().replace("\n", "").replace("restored.",
					"restored in version " + VersionControlObject.getVersionNumber()));
		}
		else {
			LOGGER.debug(" \t ++ Object: " + VersionControlObject.getSavedName() + " Successfully restored in version: "
					+ VersionControlObject.getVersionNumber());
		}
	}
	...
	public boolean restoreSpecificVersion(String ObjectName, int VersionNumber) throws IOException {
		...
		if (vcl.size() != 0) {
			...
			LOGGER.info("\t -- Error: No Version Found. Is Versioning Activated?");
			return false;
		}
	...
	public void restorePreviousVersion(String ObjectName) throws IOException {
		...
		if (vcl.size() != 0) {
			...
			LOGGER.info(" \t -- Error: No Previous Version Found. Is Versioning Activated?");
		}
		...
		while (it.hasNext() && ObjectNotFound) {
			...
			if (item.getName().equalsIgnoreCase(ObjectName)) {
				...
				if (mov.getMessageBox() != null) {
					LOGGER.info(" -- " + mov.getMessageBox().getText().toString().replace("\n", ""));
				}
				else {
					LOGGER.debug(
							" \t ++ Object: " + item.getName() + " From: " + FolderSource.fullPath() + " Successfully moved to Folder: "
									+ FolderTarget.fullPath());
				}
			}
		}
	}

	public void importObjects(String FilePathForImport, IFolder folder, boolean overwriteObject, boolean overwriteFolderLinks)
		...
		if (imp.getMessageBox() != null) {
			LOGGER.info(" -- " + imp.getMessageBox().getText().toString().replace("\n", ""));
		}
		else {
			LOGGER.debug(" \t ++ Object(s) Successfully Imported.");
		}
	}
	...
	public ArrayList<QuickSearchItem> quickSearch(String ObjectName) throws IOException {
		...
		if (exp.getMessageBox() != null) {
			LOGGER.info(" -- " + exp.getMessageBox().getText().toString().replace("\n", ""));
		}
		else {
	...
	public void exportFolder(IFolder folder, String FilePathForExport) throws IOException {
		...
		if (exp.getMessageBox() != null) {
			...
			LOGGER.debug(" \t ++ Folder Successfully Exported.");
		}
	}
	...
	public void exportFolders(IFolder[] folder, String FilePathForExport) throws IOException {
		...
		if (exp.getMessageBox() != null) {
			...
			LOGGER.debug(" \t ++ Folders Successfully Exported.");
		}
	}
	...
	public void exportObject(String ObjectName, String FilePathForExport) throws IOException {
		...
		if (exp.getMessageBox() != null) {
			...
			LOGGER.debug(" \t ++ Object(s) Successfully Exported.");
		}
	}
	...
	public void exportObjects(UC4ObjectName[] objectNames, String FilePathForExport) throws IOException {
		...
		if (exp.getMessageBox() != null) {
			...
			LOGGER.debug(" \t ++ Objects " + " Successfully Exported to File: " + FilePathForExport.toString());
		}
	}
	...
	public List<SearchResultItem> searchObjectWithFilter(String ObjectName) throws IOException {
		...
		ser.setName(ObjectName);
		/*
		 * ser.setTypeCALE(true); ser.setTypeCALL(true); ser.setTypeCITC(true); ser.setTypeCLNT(true); ser.setTypeCODE(true);
		 * ser.setTypeCONN(true); ser.setTypeCPIT(true); ser.setTypeDASH(true); ser.setTypeDOCU(true); ser.setTypeEVNT(true);
		 * ser.setTypeExecuteable(); ser.setTypeFILTER(true); ser.setTypeFOLD(true); ser.setTypeHOST(true); ser.setTypeHOSTG(true);
		 * ser.setTypeHSTA(true); ser.setTypeJOBF(true); ser.setTypeJOBF(true); ser.setTypeJOBG(true); ser.setTypeJOBP(true);
		 * ser.setTypeJOBQ(true); ser.setTypeJOBS(true); ser.setTypeJSCH(true); ser.setTypeLOGIN(true); ser.setTypePRPT(true);
		 * ser.setTypeQUEUE(true); ser.setTypeSCRI(true); ser.setTypeSERV(true); ser.setTypeSTORE(true); ser.setTypeSYNC(true);
		 * ser.setTypeTZ(true); ser.setTypeUSER(true); ser.setTypeUSRG(true); ser.setTypeVARA(true); ser.setTypeXSL(true);
		 */
		connection.sendRequestAndWait(ser);
		Iterator<SearchResultItem> it = ser.resultIterator();
		List<SearchResultItem> results = new ArrayList<SearchResultItem>();
	...
	public void replaceObject(String SourceObjectName, String TargetObjectName) throws IOException {
		...
		if (rep.getMessageBox() != null) {
			LOGGER.info(" -- " + rep.getMessageBox().getText().toString().replace("\n", ""));
		}
		else {
			LOGGER.debug(" \t ++ Object " + SourceObjectName + " Successfully Replaced by: " + TargetObjectName);
		}
	...
	public void renameObject(String SourceObjectName, String TargetObjectName, IFolder folder) throws IOException {
		...
		// this method should always only be used on exact objects and not on * or ? expressions.. the items List should always yield
		// exactly one element
		if (items.size() == 1) {
		...
		if (ren.getMessageBox() != null) {
			LOGGER.info(" -- " + ren.getMessageBox().getText().toString().replace("\n", ""));
		}
		else {
			LOGGER.debug(" \t ++ Object " + SourceObjectName + " Successfully Renamed to: " + TargetObjectName);
		}

	}

	public void renameFolder(String SourceFolderName, String TargetFolderName) throws IOException, AutomicAEApiException {
		...
		if (ren.getMessageBox() != null) {
			...
			LOGGER.debug(" \t ++ Object " + sourceFolder.fullPath() + " Successfully Renamed to: " + targetName);
		}
	...
	public void deepRenameObjects(String ExistingPatternName, String NewPatternName) throws IOException {
		...
		if (deepRename.getMessageBox() != null) {
			LOGGER.info(" -- " + deepRename.getMessageBox().toString().replace("\n", ""));
		}
		else {
			LOGGER.debug(" \t ++ Object(s) with Pattern: " + ExistingPatternName + " Successfully renamed to Pattern: " + NewPatternName);
		}
	}
	...
	public void duplicateObject(String SourceObjectName, String TargetObjectName, IFolder folder) throws IOException {
		...
		if (dup.getMessageBox() != null) {
			LOGGER.info(" -- " + dup.getMessageBox().toString().replace("\n", ""));
		}
		else {
			LOGGER.debug(" \t ++ Object: " + obj.getName() + " Successfully saved in folder " + folder);
		}
	}

	// Open an Automic Object (of any kind)
	public UC4Object openObject(String name, boolean readOnly) throws IOException {
		// LOGGER.debug(" \t ++ Opening object: "+name);
	...
	public void saveObject(UC4Object obj) throws IOException {
		// LOGGER.debug(" \t ++ Saving object: "+obj.getName()+"(Type: "+obj.getType()+")");
		SaveObject save = new SaveObject(obj);
		connection.sendRequestAndWait(save);
		if (save.getMessageBox() != null) {
			LOGGER.info("\t -- " + save.getMessageBox().getText().toString().replace("\n", ""));
		}
		LOGGER.debug(" \t ++ Object: " + obj.getName() + " Successfully saved");
	}
	...
	public void reclaimObject(String ObjectName) throws TimeoutException, IOException {
		if (ObjectName.contains("/")) {
			...
			if (req.getMessageBox() != null) {
				...
				LOGGER.debug(" \t ++ Object: " + objName + " Successfully reclaimed");
			}
		}
		else {
			UC4ObjectName objName = new UC4ObjectName(ObjectName);
	...
	public void closeObject(UC4Object obj) throws IOException {
		// LOGGER.debug(" \t ++ Closing object: "+obj.getName()+"(Type: "+obj.getType()+")");
		CloseObject close = new CloseObject(obj);
		connection.sendRequestAndWait(close);
		if (close.getMessageBox() != null) {
			LOGGER.error(" -- " + close.getMessageBox().getText().toString().replace("\n", ""));
		}
		else {
			LOGGER.debug(" \t ++ Object: " + obj.getName() + " Successfully closed");
		}
	}
	...
	public void saveAndCloseObject(UC4Object obj) throws IOException {
		...
		if (save.getMessageBox() != null) {
			...
			if (close.getMessageBox() != null) {
				...
				LOGGER.debug(" \t ++ Object: " + obj.getName() + " Successfully saved & closed");
			}
		}
	}
	...
	public int executeObjectNow(String name) throws IOException {
		...
		LOGGER.debug(" \t ++ Executing object Now: " + name);
		...
		if (execute.getMessageBox() != null || execute.getRunID() == 0) {
			if (execute.getMessageBox() != null) {
				LOGGER.error(" -- " + execute.getMessageBox().getText().toString().replace("\n", ""));
				// LOGGER.info("-- Failed to execute object:"+name + ":" +execute.getMessageBox().getText());
			}
		}
		else {
			LOGGER.debug(" \t ++ Object: " + name + "++ Successfully executed with Run ID: " + execute.getRunID());
		}
	...
	public int executeObjectOnce(String name, String timezone, DateTime startDate, DateTime logicalDate) throws IOException {
		...
		LOGGER.debug(" \t ++ Executing object Once: " + name);
	...
	public int executeObjectRecurring(String name, ExecuteRecurring recurringPattern) throws IOException {
		...
		LOGGER.debug(" \t ++ Executing object Recurring: " + name);
	...
	public void createObject(String name, Template template, IFolder fold) throws IOException {
		LOGGER.debug(" \t ++ Creating object: " + name + " of Type: " + template.getType());
		...
		if (create.getMessageBox() != null) {
			LOGGER.info(create.getMessageBox().getText().toString().replace("\n", ""));
		}
		else {
			LOGGER.debug(" \t ++ Object: " + name + " Successfully created (Type: " + template.getType() + ")");
		}
	}
	...
	public void deleteObject(String name, boolean ignoreError) throws IOException {
		...
		if (delete.getMessageBox() != null) {
			...
			if (delete.getMessageBox().getNumber() == 4006507) { // active
				...
				// connection.sendRequestAndWait(delete);
				if (delete.getMessageBox() == null) {
					LOGGER.debug(" \t ++ Object: " + name + " Successfully deleted. ");
					return;
				}
			}

			if (ignoreError) {
				LOGGER.info(delete.getMessageBox().getText().toString().replace("\n", ""));
				return;
			}
			LOGGER.info((delete.getMessageBox().getText().toString().replace("\n", "")));
		}
	...
	public void cancelObject(int rundId) throws IOException {
		LOGGER.info("Cancel RundId(" + rundId + ") ... ");
		CancelTask cancel = new CancelTask(rundId, false);
		connection.sendRequestAndWait(cancel);
		if (cancel.getMessageBox() != null) {
			LOGGER.info(cancel.getMessageBox().getText().toString().replace("\n", ""));
		}
		else {
			LOGGER.debug(" \t ++ Object with RUNID: " + rundId + " Successfully Cancelled.");
		}
	}
