...
public class Variables extends ObjectTemplate {
	...
	public Variable getJobFromObject(UC4Object object) {
	...
	public void ShowVariableContent(Variable var) {
		...
		LOGGER.info(" ++ Content for Variable: " + var.getName());
		while (it.hasNext()) {
			String key = it.next();
			LOGGER.info("  => " + key + " : " + var.get(key));
		}
	}
	...
	public void createBackendVariable(String Name, IFolder folder, String HostAgent, String Login, String Result,
		...
		LOGGER.debug(" %% Setting backend info ... ");
		...
		LOGGER.info(" ++ Backend Variable Created");
	...
	public String getVariableType(Variable obj) {
		...
		if (obj.dynamicExecVariable() != null) {
			return "EXEC";
		}
		...
		if (obj.sqlIntern() != null) {
			return "SQLI";
		}
		if (obj.sqlInternSecure() != null) {
			return "SEC_SQLI";
		}
		if (obj.sqlSecure() != null) {
			return "SEC_SQL";
		}
