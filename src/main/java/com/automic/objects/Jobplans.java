...
public class Jobplans extends ObjectTemplate {
	...
	public JobPlan getJobPlanFromObject(UC4Object object) {
		return (JobPlan) object;
	}
	...
	public WorkflowIF getIFWorkflowFromObject(UC4Object object) {
		return (WorkflowIF) object;
	}

	public WorkflowLoop getFOREACHWorkflowFromObject(UC4Object object) {
		return (WorkflowLoop) object;
	}

	public JobPlan getWorkflowFromObject(UC4Object object) {
		return (JobPlan) object;
	}
	...
	public void createLargeWorkPlan(String WorkPlanName, IFolder folder, Template template, String JobNamePrefix, int NumberOfJobs,
			int NumberOfJobsPerLine) throws IOException {
		ObjectBroker broker = getBrokerInstance();

		// Creating a few jobs
		ArrayList<String> JobNames = new ArrayList<String>();
		for (int i = 1; i <= NumberOfJobs; i++) {
			String name = broker.jobs.createRandomJobInFolder(template, "echo something", "WIN-AGENT2", "ADMIN", "CLIENT_QUEUE", folder,
					JobNamePrefix);
			JobNames.add(name);
		}
		...
		for (int i = 0; i < JobPlans.size(); i++) {
			...
			if (X == 2) {
				JobPlans.get(i).dependencies().addDependency(taskStart, TaskState.ANY_OK);
			}
	...
	public void createSimpleJobPlan(String name, String Job1Name, String Job2name) throws IOException {
		...
		LOGGER.info("Add tasks to JobPlan ... ");
		jobPlan.addTask(job1);
		jobPlan.addTask(job2);

		LOGGER.info("Set task dependencies ... ");
		job2.dependencies().addDependency(job1, TaskState.ANY_OK);

		LOGGER.info("Connect open ends with <START> and <END> ... ");
		jobPlan.closeJobPlanTasks(TaskState.ANY_OK);

		LOGGER.info("Calculate JobPlan layout ... ");
		jobPlan.format();

		LOGGER.info("Verifying calculated lnr and positions ... ");
