...
public class Agents extends ObjectTemplate {
	...
	public void startAgent(String AgentName) throws IOException {
		...
		if (req.getMessageBox() != null) {
			...
			LOGGER.debug(" ++ Agent: " + AgentName + " Successfully Started.");
		}
	}
	...
	public void disconnectAgent(AgentListItem item) throws IOException {
		...
		if (req.getMessageBox() != null) {
			...
			LOGGER.debug(" ++ Agent: " + item.getName() + " Successfully Shutdown.");
		}
	}
	...
	public void disconnectAgent(String AgentName) throws IOException {
		...
		if (req.getMessageBox() != null) {
			...
			LOGGER.debug(" ++ Agent: " + AgentName + " Successfully Shutdown.");
		}
	}
	...
	public AgentAssignment getAgentAssignmentFromObject(UC4Object object) {
		return (AgentAssignment) object;
	}
	...
	public ArrayList<AgentListItem> getAgentListWithNameAndTypeFilter(String NameFilter, String TypeFilter) throws IOException {
		...
		if (list.getMessageBox() != null) {
			LOGGER.info(" -- " + list.getMessageBox().getText().toString().replace("\n", ""));
		}
	...
	public void displayPermissionsForAgent(String AgentName) throws IOException {
		...
		while (it.hasNext()) {
			...
			logWithFormat(LOGGER, "%-40s, %-8s, %-17s, %-17s, %-12s, %-15s, %-24s, %-9s, %-7s, %-8s, %-10s, %-19s, %-16s, %-19s, %-18s\n",
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if (item.isAuthenticated()) {
				...
				while (it.hasNext()) {
					...
					logWithFormat(LOGGER,
							"%-40s, %-8s, %-17s, %-17s, %-12s, %-15s, %-24s, %-9s, %-7s, %-8s, %-10s, %-19s, %-16s, %-19s, %-18s\n",
							list.get(i).getName().toString(), item.getJclVariant(), item.getVersion(), item.getIpAddress(),
							item.getHardware(),
							item.getSoftware(), item.getSoftwareVersion(), entry.getClient(), entry.isRead(), entry.isWrite(),
							entry.isExecute(),
							item.getLicenseCategory(), item.getLicenseClass(), item.getMaxJobResources(),
							item.getMaxFileTransferResources());
				}
			}
			else {
				logWithFormat(LOGGER,
	...
	public void setAgentAuthorizationsForAllAgents(int Client, boolean Read, boolean Write, boolean Execute)
	...
	public void setAgentAuthorizations(String AgentName, int Client, boolean Read, boolean Write, boolean Execute)
			throws TimeoutException, IOException {
		...
		if (CurrentSessionClient.equals("0000")) {
			...
			LOGGER.info("-- Error! Current Client is: " + CurrentSessionClient
					+ ", You Need to be connected with Client 0000 for this operation.");
		}
	}
