...
public class Folders extends ObjectTemplate {
	...
	public IFolder getFolderFromObject(UC4Object object) {
	...
	private void addFoldersToList(ArrayList<IFolder> folderList,
		...
		if (onlyExtractFolderObjects) {
			if (myFolder.getType().equals("FOLD")) {
	...
	public IFolder getVersionControlFolder() throws IOException {
		...
		for (IFolder folder : allFolders) {
			if (folder.getType().equalsIgnoreCase("VERSCONTROL")) {
				return folder;
			}
		}
		return null;
	...
	public IFolder getNoFolderFolder() throws IOException {
		ArrayList<IFolder> allFolders = getAllFolders(false);
		for (IFolder folder : allFolders) {
			if (folder.getType().equalsIgnoreCase("NFOLD")) {
				return folder;
			}
			// for CLient 0, NOFOLD is actually CLNT.. ?!
			if (folder.getType().equalsIgnoreCase("CLNT")) {
				return folder;
			}
		}
		return null;
	...
	public IFolder getFavoritesFolder() throws IOException {
		ArrayList<IFolder> allFolders = getAllFolders(false);
		for (IFolder folder : allFolders) {
			if (folder.getType().equalsIgnoreCase("FAV")) {
				return folder;
			}
		}
		return null;
	...
	public IFolder getRecentFolder() throws IOException {
		ArrayList<IFolder> allFolders = getAllFolders(false);
		for (IFolder folder : allFolders) {
			if (folder.getType().equalsIgnoreCase("RCNT")) {
				return folder;
			}
		}
		return null;
	...
	public IFolder getRecycleBinFolder() throws IOException {
		ArrayList<IFolder> allFolders = getAllFolders(false);
		for (IFolder folder : allFolders) {
			if (folder.getType().equalsIgnoreCase("TRASH")) {
				return folder;
			}
		}
		return null;
	...
	public IFolder getTransportCaseFolder() throws IOException {
		ArrayList<IFolder> allFolders = getAllFolders(false);
		for (IFolder folder : allFolders) {
			if (folder.getType().equalsIgnoreCase("TRANSCASE")) {
				return folder;
			}
		}
		return null;
	...
	public IFolder getFolderByName(String FolderName) throws IOException, AutomicAEApiException {
		...
		if (foundFolders.size() == 1) {
			return foundFolders.get(0);
		}
		else {
			LOGGER.error(" -- Error: " + foundFolders.size() + " Folder(s) Found corresponding to name: " + FolderName);
			if (foundFolders.size() == 0) {
				LOGGER.error("  => Are you sure the Folder specified exists in the Target Client?");
			}
			else {
				for (int i = 0; i < foundFolders.size(); i++) {
					LOGGER.error("  ----> " + foundFolders.get(i).fullPath());
				}
				LOGGER.error(
						"  => Please select only one folder. HINT: Try passing the full path name. Ex: \"0002/CUSTOM.DEMOS/ARCHIVE/ABC/JOBS\" or \"SWINVM1 - 0002/CUSTOM.DEMOS/ARCHIVE/ABC/JOBS\"");
			}
			// System.exit(1);
			throw new AutomicAEApiException(" -- Error: " + foundFolders.size() + " Folder(s) Found corresponding to name: " + FolderName);
		}
	...
	public IFolder getFolderByFullPathName(String FolderName) throws IOException {
		...
		for (IFolder folder : allFolders) {
			String FullPath = "";
			// the IFolder.fullpath() method ALWAYS returns the system name before the path (ex: "AEV10 -
			// 0005/UC4.APPLICATIONS/JFORUM_BREN")
	...
	public FolderList getFolderContentByName(String FolderName) throws IOException, AutomicAEApiException {
	...
	public void deleteFolder(IFolder fold) throws IOException {
		...
		if (delete.getMessageBox() != null) {
			...
			// LOGGER.info("Failed to delete object:"+fold.fullPath());
		}
		else {
			LOGGER.debug("\t ++ Folder: " + fold.fullPath() + " Successfully Deleted.");
		}
	}
	...
	public boolean createFolderSilently(String name, IFolder fold) throws IOException {
		...
		if (create.getMessageBox() != null) {
			...
			// LOGGER.debug("\t ++ Folder: "+name+" Successfully created.");
			return true;
		}
		return false;
	}
