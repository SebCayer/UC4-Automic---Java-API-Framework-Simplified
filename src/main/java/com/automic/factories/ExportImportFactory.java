...
/**
 *
 * @author bsp this class implements copy of objects operations between different clients Ex: Copy the content of a given folder from Client
 *         1 to Client 130
 *
 */
...
public class ExportImportFactory extends FactoryTemplate {
	...
	public void CopyFolderContentBetweenClients(Connection ConnectionSource, String FolderFullPathSource, Connection ConnectionTarget,
			String FolderFullPathTarget) throws IOException, SAXException {
		...
		LOGGER.info("Target Folder is: " + folderTarget.fullPath());
		Iterator<FolderListItem> it = listTarget.iterator();
		while (it.hasNext()) {
			FolderListItem item = it.next();
			LOGGER.info("Object: " + item.getName());
		}
	}
